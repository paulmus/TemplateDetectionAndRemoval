...
//			System.out.println("###"+t.size());
		...
		for(int i = 1; i<=m ; i++){
			for(int j = 1; j<=n ; j++){
				...
				if(identicalSubtree(tree1.getChildTags()[i-1],tree2.getChildTags()[j-1])){
					
//					System.out.println("treeMatch");
...
//		System.out.println("");
	...
	private boolean identicalSubtree(TagNode _tagNode1, TagNode _tagNode2) {
		
		TagNode[] childs1 = _tagNode1.getChildTags();
		TagNode[] childs2 = _tagNode2.getChildTags();
		
		if(childs1.length != childs2.length){
			return false;
		}
		
		for( int o = 0; o<childs1.length; o++){
			if(!compareNode(childs1[o],childs2[o])){
				return false; //otherwise go to next node
			}
			
			//case primitve node
//			if(childs1[o].getChildTags().length == 0 && childs2[o].getChildTags().length == 0){
//				if(!compareNode(childs1[o],childs2[o])){
//					return false; //otherwise go to next node
//				}
//			}else{
//				if(!identicalSubtree(childs1[o],childs2[o])){
//					return false; //otherwise go to next node
//				}
//			}
		}
...
//		if(identicalNode(tree1.getChildTags()[_tagNode1-1],tree2.getChildTags()[_tagNode2-1])){
//			
//			indenticalNodesWithContent(_tagNode1-1,_tagNode2-1);
//			
//		}else{
//			return false;
//		}
...
//		System.out.println(_tagNode1.getName()+_tagNode2.getName());
		...
		if( getHashOfNode(_tagNode1) == getHashOfNode(_tagNode2) ){
			return true;
		}else{
			return false;
		}
	...
	private int getHashOfNode(TagNode _tn){
		...
		String f = _tn.getName()+_tn.getAttributeByName("id")+_tn.getAttributeByName("class");
